...
- Put the areastat package in the directory where the other packages of fido
  husky reside:
...
dos:
```sh
   d:\husky\areastat>make -f makefile.djg
```
...
- Run CMake to configure the build tree.
   ...
   ```
  This will prepare to build areastat using static libraries. If you want to build
  it using dynamic libraries, then you have to run
   ```sh
      $ cmake -H. -Bbuild
   ```
...
You're ready. Now you can install software which uses areastat.
